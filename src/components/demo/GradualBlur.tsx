...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      const conf = config as Record<string, string | number | undefined>;
      if (w <= 480 && conf["mobile" + k]) v = conf["mobile" + k];
      else if (w <= 768 && conf["tablet" + k]) v = conf["tablet" + k];
      else if (w <= 1024 && conf["desktop" + k]) v = conf["desktop" + k];
      setVal(v);
    };
...
const GradualBlur: React.FC<PropsWithChildren<GradualBlurProps>> = (props) => {
  ...
  const { hoverIntensity, animated, onAnimationComplete, duration } =
    config as Required<GradualBlurProps>;
  useEffect(() => {
